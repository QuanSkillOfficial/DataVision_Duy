...
class VectorStore:
        ...
        try:
            ...
            if self.connection_string:
                ...
                connection_string = (
                    ...
                    f"dbname={os.getenv('DB_NAME', 'datavision_db')} "
                    f"user={os.getenv('DB_USER', 'datavision')} "
                    f"password={password}"
                )
    ...
    def log_query(
        ...
        """Insert a row into rag_query_logs so v_rag_daily_metrics reflects real usage.

        Call this every time search() is used to answer a real query —
        search() alone never writes to rag_query_logs on its own.
        """
        if not (self.use_pgvector and self.connection):
            raise RuntimeError("log_query requires an active pgvector connection")

        import json

        if document_id is None and results:
            document_id = results[0].get("document_id")

        retrieved_chunk_ids = [r["chunk_id"] for r in results]
        retrieval_scores = [r.get("score", r.get("similarity_score")) for r in results]
        ...
        try:
            ...
            self.connection.rollback()
            raise RuntimeError(f"Failed to log RAG query: {e}") from e
        finally:
            cursor.close()
