...
def main():
    ...
    section("1. Tracked .env files", env_files)

    dump_files = [
        f for f in tracked_files
        if re.search(r"\.dump$|\.sql\.gz$|\.pgdump$", f)
    ]
    section("2. Tracked database dump files", dump_files)

    pycache_files = [
        f for f in tracked_files
        if re.search(r"__pycache__/|\.py[cod]$", f)
    ]
    section("3. Tracked __pycache__ / .pyc files", pycache_files)
    ...
    for f in tracked_files:
        ...
        try:
            ...
            continue
    section("4. Likely hard-coded credentials in tracked files", grep_hits)
    ...
    history_hits = sorted(set(
        ...
        if re.search(r"(^|/)\.env(\..*)?$|\.dump$|\.pgdump$", f)
    ))
    section(
        "5. .env / dump files anywhere in git history",
        history_hits,
