...
Week 7 Task 6 — CI Database Smoke Test
...
def get_connection():
    conn_string = os.getenv("DATABASE_URL") or (
        f"host={os.getenv('POSTGRES_HOST', 'localhost')} "
        f"port={os.getenv('POSTGRES_PORT', '5432')} "
        f"dbname={os.getenv('POSTGRES_DB', 'datavision_db')} "
        f"user={os.getenv('POSTGRES_USER', 'datavision')} "
        f"password={os.getenv('POSTGRES_PASSWORD', 'datavision123')}"
    )
