...
def get_git_sha(repo_path: str, override: str | None = None) -> str:
    """Return a full source SHA for evidence; never emit a placeholder SHA."""
    candidate = override or os.getenv("QS_RELEASE_SHA") or os.getenv("GITHUB_SHA")
    try:
        if not candidate:
            res = subprocess.run(
                ["git", "rev-parse", "HEAD"],
        ...
        raise RuntimeError("Unable to resolve the source SHA for fixture evidence") from exc

    if not re.fullmatch(r"[0-9a-fA-F]{40}", candidate or ""):
        raise ValueError("release SHA must be a full 40-character Git commit SHA")
    return candidate.lower()


def regenerate_fixtures(repo_root: str, release_sha: str | None = None):
    ...
    release_sha = get_git_sha(repo_root, release_sha)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Regenerate canonical prediction and UI fixtures."
    )
    parser.add_argument(
        "--repo-root",
        default=PROJECT_ROOT,
        help="Canonical repository root (defaults to the current script's repository).",
    )
    parser.add_argument(
        "--release-sha",
        default=None,
        help="Full source commit SHA. Defaults to QS_RELEASE_SHA, GITHUB_SHA, or HEAD.",
    )
    args = parser.parse_args()
    regenerate_fixtures(os.path.abspath(args.repo_root), args.release_sha)
